...
R.packages <- c("openxlsx tidyverse mice caret corrplot see smotefamily
                ...
                ggstance visdat naniar wesanderson DescTools remotes
...
We use Docker to containerize and render our Quarto R file. A **container** is a running instance on an **image** that includes the application (e.g., the programming language interpreter needed to run a workflow) and the system libraries required by an application to run. This is a powerful tool that facilitate persistent reproducibility of analytical work. Our containerized working environment can be accessed via [Dockerhub](<https://hub.docker.com/repository/docker/mohamadbazzi/turtle-image/general>).
...
lapply(irmi_subset, function(x) cor(x[,"Humerus.Length"],pImp[,"V1"])) |> 
  ...
  dplyr::select("Var")
  # gghistostats(x = "Var", bf.message = F,bf.prior = F,results.subtitle = F, xlab = "Pearson correlation", title = "Humerus variable") +
  # theme(aspect.ratio = 1)
...
lapply(irmi_subset, function(x) cor(x[,"Ulna.Length"],pImp[,"V2"])) |> 
  ...
  as.data.frame() |> setNames(nm = "Var") |>
  dplyr::select("Var") 
  # gghistostats(x = "Var", bf.message = F,bf.prior = F,results.subtitle = F, xlab = "Pearson correlation", title = "Ulna variable") +
  # theme(aspect.ratio = 1)
...
lapply(irmi_subset, function(x) cor(x[,"Digit.III"],pImp[,"V3"])) |> 
  ...
  as.data.frame() |> setNames(nm = "Var") |>
  dplyr::select("Var") 
  # gghistostats(x = "Var", bf.message = F,bf.prior = F,results.subtitle = F, xlab = "Pearson correlation", title = "Digit III variable") +
  # theme(aspect.ratio = 1)
```
...
#### Variance inflation factor index {#vif}
...
nnet:::predict.multinom(mod.3,newdata = pImDd,type = "prob") |> 
  as.data.frame() |> 
...
performance_index |>
  ...
  labs(y = "Value", x = NULL,title = "Performance",
       subtitle = "Smote vs unbalanced habitat data") +
  scale_color_manual(values = c("#006B81","#E04F39")) +
  scale_fill_manual(values =  c("#006B81","#E04F39")) +
  theme_minimal() + 
...
LDA using the R package `MASS` v. 7.3.60.0.1.
...
#### LDA predictions (Phylogenetic Imputed)

...
R.packages <- c("openxlsx","tidyverse","mice","caret","corrplot","see","smotefamily",
                "ggfortify","GGally","MASS","MuMIn","DMwR2","tidyr","kernlab",
                ...
                "magrittr","checkpoint","data.table","DiagrammeR","modelsummary")

invisible(lapply(R.packages,library,character.only = TRUE)) |> suppressMessages()
...
We use Docker to containerize and render our Quarto R file. A **container** is a running instance on an **image** that includes the application (e.g., the programming language interpreter needed to run a workflow) and the system libraries required by an application to run. This is a powerful tool that facilitate persistent reproducibility of analytically work.
...
```{r fig.cap="Normality test, data distribution, and QQ-plots. Graphic results reveal an overall right-skewness in each un-transformed numeric variable. P-values are based on a Shapiro-Wilk Normality Test"}
# Plot using a qqplot.
...
SMOTE synthesizes new minority instances between existing minority instances. After synthesizing new minority records, the imbalance shrinks from 7 semi-aquatic to 21 semi-aquatic instances.
...
performance_index |>
  ...
  geom_text(aes(x = variable, y = value,label = round(value,digits = 2)),
